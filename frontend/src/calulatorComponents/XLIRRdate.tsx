...
export const XLIRRdate: React.FC<IProps> = ({
  ...
  for (let i = 0; i < totalTenor; i++) {
    ...
    totalAmount -= ppl / totalTenor;
    const cashflow = ppl / totalTenor + monthlyInterest;
        ...
        <tbody>
          ...
          {repeatedDates.map((entry: any, index: any) => (
            <tr key={index}>
              ...
              <td>-</td>
              <td>-</td>
              <td>{(ppl / totalTenor + entry.interest).toFixed(2)}</td>
