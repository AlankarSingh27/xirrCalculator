...
import LenderTable from "../database/LenderSchema";
 ...
 */
export const getAllLenders = async (request: Request, response: Response) => {
    try {
        ...
        let contacts: IContact[] | undefined = await LenderTable.find({user: new mongoose.Types.ObjectId(userId)}); // select * from contacts;
        if (contacts) {
 ...
 */
export const getLender = async (request: Request, response: Response) => {
    try {
        const userObj: any = request.headers['user-data'];
        const userId = userObj.id;
        const mongoUserId = new mongoose.Types.ObjectId(userId);
        ...
        if (contactId) {
            ...
            const contact: IContact | undefined | null = await LenderTable.findOne({
                _id: mongoContactId,
                user: mongoUserId
            });
            if (!contact) {
...
/**
 ...
 @url : http://localhost:9000/contacts/
 */
export const createLender = async (request: Request, response: Response) => {
    ...
    try {
        ...
        // let contact = await LenderTable.findOne({mobile: mobile});
        // if (contact) {
        ...
        theContactObj = await new LenderTable(theContactObj).save();
        if (theContactObj) {
...
/**
 ...
 @url : http://localhost:9000/contacts/:contactId
 */
export const updateLender = async (request: Request, response: Response) => {
    ...
    try {
        ...
        const mongoContactId = new mongoose.Types.ObjectId(contactId);
        let contact: IContact | null | undefined = await LenderTable.findOne({
            _id: mongoContactId,
            user: mongoUserId
        });
        if (!contact) {
        ...
        theContactObj = await LenderTable.findByIdAndUpdate(mongoContactId, {
            $set: theContactObj
        }, {new: true})
        if (theContactObj) {
            return response.status(200).json(theContactObj);
...
/**
 ...
 @url : http://localhost:9000/contacts/:contactId
 */
export const deleteLender = async (request: Request, response: Response) => {
    try {
        const userObj: any = request.headers['user-data'];
        const userId = userObj.id;
        const mongoUserId = new mongoose.Types.ObjectId(userId);
        ...
        if (contactId) {
            ...
            let theContact: IContact | null = await LenderTable.findByIdAndDelete(mongoContactId);
            if (theContact) {
